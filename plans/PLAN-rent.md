...
The app's sidebar radio is 5 numbered screens, visited in order, mirroring these beats exactly: **"1 · The P&L"**, **"2 · Hire"**, **"3 · Fire"**, **"4 · Proof"**, **"5 · Autopilot"**. A muted presenter-cue line (time window + opening line) sits at the top of every screen as a rehearsal card.

**0:00–0:20 — Screen "1 · The P&L", pre-loaded from the measured `show1` run (live mode, `REPLAY_MODE` off).** Pre-demo staging (Task 7) already ran `seed.py --restore-active`, so all 12 bundles — INCLUDING B09/B10 — show ACTIVE badges right now; the FIRE/PRUNE clicks on screen 3 below are a real flip, not a re-display. "12 memory bundles seeded into EverOS, one real support/ops question stream, 8 exact-answer questions. Every bundle pays rent for its seat and only earns if it's the actual evidence behind a right answer — here's the P&L." Headline NET `$` metric already filled in.

**0:20–1:00 — Walk the ranked table.** Point to the top earner: "Its rent is what it cost to sit in context; its earnings are the tokens we saved on the one question it actually answered — net is the difference, and it's real, not a guess." Point to the gray idle panel: "these two never entered prompt context at all. Zero rent, zero earned — audit candidates, not part of the P&L." (The two red "not paying rent" decoys stay off-screen for now — they get their reveal on screen 3, right before they get fired.)

**1:00–1:30 — Switch to screen "2 · Hire." Watch a memory get hired (live lifecycle beat).** Type one new fact into "Feed the agent" — the audience watches EverOS extract and store it; a "memory born" panel shows what it distilled, and it enters the board ACTIVE and in the red. A freeform query (same screen) pulls it back and its FIRST rent charge lands on the leaderboard as a `+$` chip. "Every memory starts life in the red — it has to earn its seat. These two [B09/B10] never did." (If extraction stalls past ~10s, pivot to the pre-fed Initech memory — same beat, no stumble; in `REPLAY_MODE` this fallback panel is what screen 2 always shows, since feeding is a live-only beat.)

**1:30–1:45 — Switch to screen "3 · Fire." Reveal the red panel, then fire the freeloaders, live.** "These two got pulled into context multiple times each — the panel shows exactly how many — and every single time they charged rent and earned nothing. That's a genuinely negative number, computed the same way as everyone else's, not a label we chose. Which one dies first? You choose." Ask the room, click its `FIRE` button — real SQL flips `active=false` for that one bundle on screen (a genuine ON→OFF transition, not a re-reveal), then `PRUNE` fires the rest. "No fake delete, EverOS doesn't even have a per-memory delete API — this is the exact mechanic it supports, and the seats they vacate stay empty, nothing backfills them." Before/after estimated prompt-input cost-per-query tiles count up, amber → green, sourced from the `show1` post-prune run captured before the show.

**1:45–2:50 — Switch to screen "4 · Proof." Regression suite, replayed, then one query, then receipts.** "Our fixed regression set retained an identical score at lower cost" — 8 PASS rows animate from the captured post-prune run. On-screen label: "measured pre-show run (replayed) — zero live calls right now." Then the presenter picks a scripted question from the selectbox and runs it against the now-pruned registry. Real `ai_complete` call, real token count, right answer, on screen — labeled LIVE (or, if `REPLAY_MODE` had to come on because of a connectivity hiccup earlier in the demo, labeled REPLAYED without breaking stride — same code path, same UI, the audience shouldn't be able to tell from the flow that anything changed). The receipts panel underneath corroborates with billed-credits metering for the same run.

**2:50–3:00 — Switch to screen "5 · Autopilot." Close.** The activity feed shows the same prune decision the room just made, plus an on-demand "Run audit cycle now" for a live verify→prune→verify loop. "Context is capital. We built the P&L for it — and it does this by itself."
