...
**0:50–1:20 — Red panel: "not paying rent."** "These two got pulled into context multiple times each — the panel shows exactly how many — and every single time they charged rent and earned nothing. That's a genuinely negative number, computed the same way as everyone else's, not a label we chose." (Sets up the scoreboard moment next.)

**1:20–1:40 — Click PRUNE, live.** Their badges said ACTIVE a second ago — real SQL now flips `active=false` for those two bundles on screen, a genuine ON→OFF transition happening live, not a re-reveal of an already-pruned state. "No fake delete, EverOS doesn't even have a per-memory delete API — this is the exact mechanic it supports, and the seats they vacate stay empty, nothing backfills them." Before/after estimated prompt-input cost-per-query tiles animate, amber → green, sourced from the `show1` post-prune run captured before the show.

**1:40–2:20 — Regression suite, replayed.** "Our fixed regression set retained an identical score at lower cost" — 8 PASS rows animate from the captured post-prune run. On-screen label: "measured pre-show run (replayed) — zero live calls right now."
